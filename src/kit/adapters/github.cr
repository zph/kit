  ...
  module Github
    class API
      ...
      def asset_urls_by_release_tag_name(tag_name)
        # TODO: rewrite this in native Crystal to avoid curl dependency
        stdout, stderr, process = Kit::POpen.call("curl", ["#{ENDPOINT}/repos/#{@org}/#{@repo}/releases/tags/#{tag_name}"])
        h = JSON.parse(stdout.to_s).as_h
        h["assets"].as_a.map { |r| r["browser_download_url"].to_s }
      end

      def download_link(tag_name : String)
        urls = asset_urls_by_release_tag_name(tag_name)
        matches = urls.grep(OS.platform.to_regex)
        if matches.size == 1
          matches.first
        else
          # TODO: consider how to do this for platform, but right now we're ignoring 86x systems and hoping for 64
          matches.grep(/64/).first
        end
      end
    end
  end
